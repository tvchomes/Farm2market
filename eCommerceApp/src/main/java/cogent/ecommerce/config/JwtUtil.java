 ...
 * 
 * import java.util.Arrays; import java.util.Collection; import java.util.Date;
 ...
 * import io.jsonwebtoken.Jws; import io.jsonwebtoken.Jwts; import
 ...
 * 
 * private String secret; private int jwtExpirationInMs;
 * 
 * @Value("${jwt.secret}") public void setSecret(String secret) { this.secret =
 * secret; }
 * 
 * @Value("${jwt.expirationDateInMs}") public void setJwtExpirationInMs(int
 * jwtExpirationInMs) { this.jwtExpirationInMs = jwtExpirationInMs; }
 * 
 ...
 * Date(System.currentTimeMillis() + jwtExpirationInMs))
 * .signWith(SignatureAlgorithm.HS512, secret).compact();
 * 
 * }
 * 
 * public boolean validateToken(String authToken) { try { Jws<Claims> claims =
 * Jwts.parser().setSigningKey(secret).parseClaimsJws(authToken); return true; }
 * catch (SignatureException | MalformedJwtException | UnsupportedJwtException |
 * IllegalArgumentException ex) { throw new
 * BadCredentialsException("INVALID_CREDENTIALS", ex); } catch
 * (ExpiredJwtException ex) { throw ex; } }
 * 
